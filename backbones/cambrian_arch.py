...
class CambrianMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # @tcm: track starting and ending positions of video tokens sequence
        video_sequence_ranges = []
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            vid_start_pos, vid_end_pos = (-1, -1)
            assert num_images == 1, "For HCMUS Thesis, we only support 1 video per batch"
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    vid_start_pos = cur_new_input_embeds[-1].shape[0]
                    vid_end_pos = vid_start_pos + cur_image_features.shape[0]
            ...
            assert vid_start_pos != -1 and vid_end_pos != -1, "Video sequence range not found"
            video_sequence_ranges.append((vid_start_pos, vid_end_pos))
